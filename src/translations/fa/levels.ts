const levels = {
    ...
    "intro.level1.hint2":
        "این کار یک پوشه مخفی .git می‌سازد — همان پوشه *خودِ* مخزن است؛ جایی که Git هر نسخه را نگه می‌دارد",
    ...
    "intro.level1.story.narrative":
        "به شغل جدیدتان به‌عنوان توسعه‌دهنده در TechStart خوش آمدید! من Alex هستم، سرپرست تیم شما.\n\nپیش از آنکه حتی یک دستور تایپ کنیم: Git چیست و چرا هر تیمی بر استفاده از آن پافشاری می‌کند؟\n\nتصور کنید امروز هر دوی ما یک فایل را ویرایش می‌کنیم. شما `login-final.js` را برای من می‌فرستید، من `login-final-v2.js` را برایتان برمی‌گردانم، و تا جمعه هیچ‌کس نمی‌داند کدام نسخه کار می‌کند — یا اینکه فایل روز دوشنبه، وقتی هنوز کار می‌کرد، چه شکلی بود.\n\nGit یک سیستم کنترل نسخه است و دقیقاً همین مشکل را حل می‌کند. تاریخچه کامل پروژه شما را نگه می‌دارد: هر تغییر، اینکه چه کسی آن را انجام داده، و راهی برای بازگشت به هر وضعیت پیشین. همچنین به چند نفر امکان می‌دهد روی یک کد کار کنند، بدون آنکه کار یکدیگر را بازنویسی کنند.\n\nGit این تاریخچه را در یک مخزن ذخیره می‌کند — پوشه معمولی پروژه شما به‌همراه یک پوشه مخفی `.git` که هر نسخه در آن نگهداری می‌شود. ساختن آن فقط یک دستور لازم دارد: `git init`.",
    "intro.level1.story.realWorldContext":
        'Git استاندارد کنترل نسخه است: تقریباً هر تیم نرم‌افزاری از آن استفاده می‌کند و اولین ابزاری است که برای یک پروژه جدید راه‌اندازی می‌شود. بدون آن، پرسش "چه کسی این را چه زمانی و چرا تغییر داد؟" بی‌پاسخ می‌ماند.',
    ...
    "intro.level3.hint1":
        "دستور `git clone` را با یک آدرس واقعی اجرا کنید، مثلاً `git clone https://github.com/octocat/Hello-World.git`",
    "intro.level3.hint2": "پس از کلون کردن، از `cd` برای ورود به پوشه مخزن استفاده کنید",
    "intro.level3.hint3":
        "باید یک آدرس واقعی مانند https://github.com/user/repo.git باشد — جای‌نگهداری که داخل کروشه‌های زاویه‌ای نوشته شده باشد پذیرفته نمی‌شود",
    ...
    "files.level1.story.narrative":
        '"سلام!" Sarah، همکار شما، صدا می‌زند. "وقت آن است یاد بگیرید Git واقعاً کار شما را چطور ذخیره می‌کند — چون ویرایش یک فایل کافی نیست."\n\nاو سه جعبه روی وایت‌برد می‌کشد:\n\n1. **پوشه شما** — جایی که فایل‌ها را ویرایش می‌کنید. Git آن را می‌بیند، اما خودش چیزی ذخیره نمی‌کند.\n2. **ناحیه استیجینگ** — فهرست تغییراتی که می‌خواهید در ذخیره بعدی شما باشند.\n3. **مخزن** — تاریخچه دائمی همه ذخیره‌ها.\n\nاو می‌گوید: "یک کامیت همیشه فقط چیزی را ثبت می‌کند که در ناحیه استیجینگ است. به همین دلیل نمی‌توانید بعد از ویرایش چیزی مستقیم کامیت کنید: Git فرض نمی‌کند همه تغییراتی که داده‌اید به هم مربوط‌اند. شما تصمیم می‌گیرید. `git add <file>` یک تغییر را به استیجینگ می‌برد و `git add .` همه چیز را در پوشه فعلی برمی‌دارد."',
    "files.level1.story.realWorldContext":
        "استیجینگ همان مرحله‌ای است که تازه‌کارها از آن می‌گذرند و توسعه‌دهندگان باتجربه به آن تکیه می‌کنند. دو چیز بی‌ربط را درست کرده‌اید؟ آن‌ها را جداگانه استیج و کامیت کنید تا تاریخچه خوانا بماند — و بعداً هر تغییر به‌تنهایی قابل برگرداندن باشد.",
    ...
    "files.level2.story.narrative":
        '"کار خوبی بود!" Alex وقتی پیشرفت شما را می‌بیند می‌گوید. "تغییرات شما استیج شده‌اند. حالا آن‌ها را دائمی کنید."\n\nاو توضیح می‌دهد: "`git commit` هر چیزی را که در ناحیه استیجینگ است برمی‌دارد و آن را به‌عنوان یک عکس لحظه‌ای — همراه با نام شما، زمان و یک پیام — در تاریخچه می‌نویسد. پس از آن ناحیه استیجینگ دوباره خالی است و آماده دسته بعدی."\n\n"دو چیز ارزش دانستن دارد. کامیتی که هیچ چیز استیج‌شده‌ای نداشته باشد هیچ کاری انجام نمی‌دهد — Git به شما می‌گوید چیزی برای کامیت کردن وجود ندارد، و این تقریباً همیشه یعنی یک `git add` جا افتاده است. و پیام کاغذبازی نیست: `-m "..."` راهی است که نفر بعدی — که معمولاً خود شما سه ماه بعد هستید — می‌فهمد چرا این تغییر وجود دارد."',
