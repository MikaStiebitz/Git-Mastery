...
describe("Git Basic Commands", () => {
    ...
    describe("CommitCommand", () => {
        ...
        it("should require a commit message", () => {
            ...
            const commitCmd = new CommitCommand();
            const output = commitCmd.execute({ args: [], flags: {}, positionalArgs: [] }, context);
        ...
        it("should fail when nothing is staged", () => {
            ...
            // Real Git reports the branch first, then what is (not) staged.
            expect(output[0]).toContain("On branch");
            expect(output.join("\n")).toMatch(/nothing to commit|no changes added to commit/);
        });
